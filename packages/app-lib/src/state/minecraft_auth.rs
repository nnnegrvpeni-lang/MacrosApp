...
pub async fn login_offline(
    ...
    if clean_name.is_empty() {
        return Err(crate::ErrorKind::InputError(
            "Minecraft username cannot be empty".to_string(),
        )
        .as_error());
    }

    let uuid = generate_offline_uuid(clean_name);
...
#[derive(Deserialize, Debug)]
struct ElyByAuthResponse {
    #[serde(rename = "accessToken", default)]
    access_token: String,
    #[serde(rename = "selectedProfile")]
    selected_profile: Option<ElyByProfile>,
    #[serde(default)]
    error: Option<String>,
...
pub async fn login_elyby(
    ...
    let clean_login = login_or_email.trim();
    if clean_login.is_empty() || password.is_empty() {
        return Err(crate::ErrorKind::InputError(
            "Ely.by login and password cannot be empty".to_string(),
        )
        .as_error());
    }

    let client_token = Uuid::new_v4().simple().to_string();
    let body = ElyByAuthRequest {
        agent: ElyByAgent {
            name: "Minecraft",
            version: 1,
        },
    ...
    let res = client
        ...
        .map_err(|e| {
            crate::ErrorKind::OtherError(format!(
                "Failed to connect to Ely.by auth server: {e}"
            ))
        })?;

    let status = res.status();
    let response_text = res.text().await.unwrap_or_default();
    tracing::info!("Ely.by auth response status {status}: {response_text}");

    let response: Result<ElyByAuthResponse, _> = serde_json::from_str(&response_text);
    let response = match response {
        Ok(r) => {
            if !status.is_success() || r.error.is_some() {
                let msg = r
                    .error_message
                    .or(r.message)
                    .or(r.error)
                    .unwrap_or_else(|| "Invalid Ely.by username or password".to_string());
                return Err(crate::ErrorKind::InputError(msg).as_error());
            }
            r
        }
        Err(_) => {
            if !status.is_success() {
                return Err(crate::ErrorKind::InputError(format!(
                    "Ely.by server error (status {status}): {response_text}"
                ))
                .as_error());
            }
            return Err(crate::ErrorKind::OtherError(format!(
                "Invalid response format from Ely.by: {response_text}"
            ))
            .as_error());
        }
    };
    ...
    let profile = response.selected_profile.ok_or_else(|| {
        crate::ErrorKind::OtherError(
            "No Ely.by Minecraft profile found for this account".to_string(),
        )
    })?;

    let uuid = Uuid::parse_str(&profile.id)
        .unwrap_or_else(|_| generate_offline_uuid(&profile.name));

    let skin_url = Url::parse(&format!(
    ...
    let credentials = Credentials {
        ...
        refresh_token: format!("elyby:{clean_login}"),
        expires: Utc::now() + Duration::days(36500),
        active: true,
    };
...
impl Credentials {
    ...
    pub fn build_offline_or_elyby_profile(&self) -> MinecraftProfile {
        let mut profile = self.offline_profile.clone();
        if self.is_elyby() {
        ...
        } else if profile.skins.is_empty() {
            if let Ok(default_skin) = crate::api::minecraft_skins::get_fallback_default_skin() {
                profile.skins.push(MinecraftSkin {
                    id: Uuid::new_v4(),
                    state: MinecraftCharacterExpressionState::Active,
                    url: Arc::clone(&default_skin.texture),
                    texture_key: Some(Arc::clone(&default_skin.texture_key)),
...
impl MinecraftProfile {
    ...
    pub fn current_skin(&self) -> crate::Result<&MinecraftSkin> {
        ...
        Err(ErrorKind::OtherError("No active skin found".into()).into())
    }
