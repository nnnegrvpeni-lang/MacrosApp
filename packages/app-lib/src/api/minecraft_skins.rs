...
use crate::{
    ErrorKind, State,
    state::{
        MinecraftCharacterExpressionState, MinecraftProfile, MinecraftSkin,
        minecraft_skins::{
    ...
    let profile = if selected_credentials.is_offline() || selected_credentials.is_elyby() {
        ...
        if let Some(data_url) = png_util::blob_to_data_url(&texture_blob) {
            profile.skins = vec![MinecraftSkin {
                ...
                url: data_url,
                texture_key: Some(Arc::from(local_texture_key)),
                variant,
                name: Some("Custom Skin".to_string()),
            }];
        }
        Arc::new(profile)
    } else {
...
async fn equip_skin_now(
    ...
    let profile = if selected_credentials.is_offline() || selected_credentials.is_elyby() {
        ...
        profile.skins = vec![MinecraftSkin {
            ...
            texture_key: Some(Arc::clone(&skin.texture_key)),
            variant: skin.variant,
            name: skin.name.as_deref().map(ToString::to_string),
        }];
        Arc::new(profile)
    } else {
        let profile = mojang_api::MinecraftSkinOperation::equip(
