...
This document provides instructions for AI assistants when creating Storybook stories for Vue components in the `@macros/ui` package.
...
Import icons from `@macros/assets`:

```typescript
import { SearchIcon, ChevronDownIcon } from '@macros/assets'
...
### 1. Don't Import from `@macros/ui` in Components
