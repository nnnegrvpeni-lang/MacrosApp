...
import { RotateCounterClockwiseIcon, SearchIcon } from '@macros/assets'
...
<template>
	...
	<div v-if="ctx.showProjectTypeTabs.value" class="flex items-center justify-between gap-3 w-full">
		<NavTabs
		...
		<ProjectCardList v-else :layout="ctx.effectiveLayout.value">
			...
			<template v-else>
				<ProjectCard
					...
					:title="result.name || (result as any).title"
					:icon-url="result.icon_url ?? undefined"
					:author="{
					...
					:summary="result.summary || (result as any).description"
