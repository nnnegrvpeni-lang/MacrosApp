    ...
    builder = builder
        .plugin(tauri_plugin_single_instance::init(|app, args, _cwd| {
            for arg in &args {
                ...
                if clean.contains("mra_") || clean.contains("code=") {
                    ...
                    break;
                } else if clean.starts_with("modrinth:") || clean.starts_with("macros:") || clean.ends_with(".mrpack") {
                    tracing::info!("Handling command-line deep link: {clean}");
                    let payload = clean.to_string();
        ...
        .plugin(
            tauri_plugin_window_state::Builder::default()
                ...
                .with_denylist(&["signin", "modrinth_signin"])
        ...
        .setup(|app| {
            ...
            for arg in std::env::args().skip(1) {
                ...
                if clean.contains("mra_") || clean.contains("code=") {
                    ...
                    tauri::async_runtime::spawn(api::utils::handle_command(clean));
                    break;
                }
            }
            ...
            #[cfg(target_os = "macos")]
            {
                ...
                app.listen("deep-link://new-url", move |url| {
                    ...
                    if let Some(code) = extract_auth_code_from_link(&actual_request) {
                        api::oauth_utils::auth_code_reply::submit_auth_code(code);
                    }
            ...
            #[cfg(any(target_os = "windows", target_os = "linux"))]
            {
                ...
                let _ = app.deep_link().on_open_url(move |event| {
                    for url in event.urls() {
                        let clean = url.as_str();
                        tracing::info!("Handling deep link on_open_url: {clean}");
                        if let Some(code) = extract_auth_code_from_link(clean) {
                            api::oauth_utils::auth_code_reply::submit_auth_code(code);
                        }
                        let payload = clean.to_string();
                        tauri::async_runtime::spawn(api::utils::handle_command(
            ...
            app.listen("deep-link://new-url", |url| {
                ...
                let actual_request = serde_json::from_str::<Vec<String>>(&request)
                    .ok()
                    .and_then(|mut x| if !x.is_empty() { Some(x.remove(0)) } else { None })
                    .unwrap_or(request);
...
fn extract_auth_code_from_link(link: &str) -> Option<String> {
