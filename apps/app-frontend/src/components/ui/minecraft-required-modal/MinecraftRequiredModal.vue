	...
	<NewModal ref="modal" :header="formatMessage(messages.header)" max-width="544px" no-padding>
		...
		<div class="flex flex-col gap-4 px-6 pb-6">
			<div class="flex flex-col gap-2">
				<Button type="colored" color="brand" class="w-full" size="lg" @click="addOffline">
					<UserPlusIcon />
			...
			<div class="grid grid-cols-2 gap-2">
				...
				<Button type="outlined" :disabled="loadingSignIn" @click="signIn">
...
const messages = defineMessages({
	...
	addElybyAccount: {
		id: 'minecraft-required.add-elyby-account',
...
function addOffline() {
	modal.value?.hide()
	accountsCard.value?.openOfflineModal()
}

function addElyby() {
	modal.value?.hide()
	accountsCard.value?.openElybyModal()
}
