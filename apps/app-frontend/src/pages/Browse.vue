...
async function search(requestParams: string) {
	...
	if (searchSource.value === 'curseforge' && !isServer) {
		try {
			const gameVersion =
			...
			const cfResult = await searchCurseForge({
				...
				gameVersion,
				loader: loader && loader !== 'vanilla' ? loader : undefined,
				page: searchState.currentPage.value,
				pageSize: searchState.maxResults.value,
			})

			for (const hit of cfResult.hits) {
				queryClient.setQueryData(['projects', 'summary', hit.project_id], hit)
			}

			return {
				projectHits: cfResult.hits,
				serverHits: [],
				total_hits: cfResult.total_hits,
				per_page: cfResult.hits_per_page,
			}
		} catch (e) {
...
<template>
	<div class="flex flex-col gap-3 p-6">
		<BrowsePageLayout>
			<template #nav-right>
				<div v-if="!isServerContext" class="flex items-center">
					<nav
						class="relative flex w-fit rounded-full bg-bg-raised p-1 text-xs sm:text-sm font-bold card-shadow border border-solid border-surface-4"
					>
						<button
							type="button"
							class="button-animation z-[1] flex flex-row items-center gap-2 px-4 py-2 rounded-full border-0 cursor-pointer transition-all text-xs sm:text-sm font-bold"
							:class="
								searchSource === 'modrinth'
									? 'bg-button-bgSelected text-brand-contrast'
									: 'bg-transparent text-secondary hover:text-contrast'
							"
							@click="switchSearchSource('modrinth')"
						>
							<span
								class="h-2 w-2 rounded-full"
								:class="searchSource === 'modrinth' ? 'bg-white' : 'bg-brand'"
							></span>
							<span>Modrinth</span>
						</button>
						<button
							...
							:class="
								searchSource === 'curseforge'
									? 'bg-[#f16436] text-white shadow-sm'
									: 'bg-transparent text-secondary hover:text-contrast'
							"
							@click="switchSearchSource('curseforge')"
						>
							<CurseForgeIcon class="size-4 fill-current" />
